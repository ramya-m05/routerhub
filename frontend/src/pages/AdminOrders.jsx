...
const STATUS_CONFIG = {
  ...
  processing:      { color: "#0891b2", bg: "#ECFEFF", label: "Processing",      icon: "⚙️" },
  shipped:         { color: "#7c3aed", bg: "#F5F3FF", label: "Shipped",         icon: "🚚" },
  out_for_delivery:{ color: "#ea580c", bg: "#FFF7ED", label: "Out for Delivery",icon: "🛵" },
  delivered:       { color: "#16a34a", bg: "#F0FDF4", label: "Delivered",       icon: "📦" },
  cancelled:       { color: "#dc2626", bg: "#FEF2F2", label: "Cancelled",       icon: "❌" },
  returned:        { color: "#6b7280", bg: "#F3F4F6", label: "Returned",        icon: "↩️" },
};
...
function AdminOrders() {
  ...
  // FILTER — supports both old userId and new user field
  const filtered = useMemo(() => {
    return orders.filter(o => {
      const userName = o.user?.name || o.userId?.name || "";
      const matchSearch =
        search === "" ||
    ...
    <div style={{ background: "#F7F7F5", minHeight: "100vh", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={styles.inner}>
        ...
        <div style={styles.toolbar}>
          ...
          <select value={filterPayment} onChange={e => setFilterPayment(e.target.value)} style={styles.filterSelect}>
            <option value="all">All Payments</option>
            <option value="online">Online</option>
            <option value="cod">COD</option>
          </select>

          <span style={styles.resultCount}>{filtered.length} order{filtered.length !== 1 ? "s" : ""}</span>
        </div>
        ...
        <div style={styles.ordersList}>
          {filtered.map(order => {
            ...
            // ✅ FIX: support both old userId and new user field
            const customerName  = order.user?.name  || order.userId?.name  || "Unknown Customer";
            const customerEmail = order.user?.email || order.userId?.email || "—";
              ...
              <div key={order._id} style={styles.orderCard}>
                ...
                <div style={styles.orderRow}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={styles.userName}>{customerName}</p>
                  ...
                  <div style={{ minWidth: "100px", textAlign: "right" }}>
                    ...
                    <span style={{
                      ...
                      color: order.paymentMode === "cod" ? "#555" : "#cc8800", letterSpacing: "0.5px"
                    }}>
                  ...
                  <div style={{ minWidth: "140px" }}>
                    <span style={{
                      ...
                      fontWeight: "800", background: cfg.bg, color: cfg.color, letterSpacing: "0.3px"
                    }}>
                      {cfg.icon} {cfg.label}
                    </span>
                  </div>
                ...
                {isExpanded && (
                  <div style={styles.expandedSection}>
                    <div style={styles.expandedGrid}>
                            ...
                            <div key={i} style={styles.expandedItem}>
                              {item.image && (
                                <img src={item.image} alt={item.name}
                                  style={{ width: "34px", height: "34px", objectFit: "cover", borderRadius: "6px", border: "1px solid #eee", flexShrink: 0 }} />
                              )}
                      ...
                      {/* DELIVERY INFO */}
                      <div style={styles.expandedBlock}>
                        <p style={styles.expandedLabel}>Customer</p>
                        <p style={styles.expandedValue}>{customerName}</p>
                        <p style={{ ...styles.expandedValue, color: "#888", fontSize: "12px" }}>{customerEmail}</p>

                        <p style={{ ...styles.expandedLabel, marginTop: "16px" }}>Delivery Address</p>
                          ...
                          {order.paymentMode === "cod" && (
                            <>
                              ...
                              <div style={styles.payRow}>
                                ...
                                <span style={{ fontWeight: "700", color: "#cc8800" }}>
                                  ₹{order.amountDueOnDelivery?.toLocaleString() || "—"}
                                </span>
                              </div>
                            </>
                          )}
